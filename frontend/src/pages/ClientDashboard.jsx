...
export default function ClientDashboard() {
  ...
  if (showSetup && !selectedHouse?.rooms?.length) {
    return (
      <div className="page-container">
        <h1 className="page-title">🏠 Set Up Your Smart Home</h1>
        <p className="page-subtitle">Let's get your house configured step by step</p>
        ...
        <div className="card" style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          {setupStep === 2 && (
            <div>
              ...
              <form onSubmit={handleAddRoom} className="flex-gap" style={{ marginBottom: 20, flexWrap: 'wrap' }}>
                <input className="form-input" style={{ flex: 1, minWidth: 150 }} placeholder="Room name" value={newRoom.name} onChange={e => setNewRoom({...newRoom, name: e.target.value})} required />
                <select className="form-select" style={{ width: 160 }} value={newRoom.type} onChange={e => setNewRoom({...newRoom, type: e.target.value})}>
              ...
              {selectedHouse?.rooms?.length > 0 && (
                <div className="room-list">
                  {selectedHouse.rooms.map(r => (
                    <div key={r.id} className="room-chip" style={{ background: r.color }}>
                      {r.name} <span style={{ opacity: 0.6 }}>({r.type})</span>
                    </div>
                  ))}
                </div>
              )}
              <div className="flex-between" style={{ marginTop: 20 }}>
                <button className="btn btn-secondary" onClick={() => setSetupStep(1)}>← Back</button>
                <button className="btn btn-primary" onClick={() => setSetupStep(3)} disabled={!selectedHouse?.rooms?.length}>Next →</button>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <form onSubmit={handleAddBoard}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="Living Room Board" value={newBoard.name} onChange={e => setNewBoard({...newBoard, name: e.target.value})} required />
                </div>
                <div className="form-group">
                ...
                <button type="submit" className="btn btn-primary btn-sm" style={{ marginBottom: 16 }}>+ Register Board</button>
              </form>
              <div className="flex-between">
                <button className="btn btn-secondary" onClick={() => setSetupStep(2)}>← Back</button>
            ...
            <div>
              ...
              <form onSubmit={handleAddDevice}>
                <div className="grid-2">
                  <div className="form-group">
                    <label className="form-label">Room</label>
                    <select className="form-select" value={newDevice.room_id} onChange={e => setNewDevice({...newDevice, room_id: e.target.value})} required>
                      <option value="">Select room...</option>
                      {selectedHouse?.rooms?.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
                    ...
                    <select className="form-select" value={newDevice.board_id} onChange={e => setNewDevice({...newDevice, board_id: e.target.value})} required>
                      ...
                      {selectedHouse?.rooms?.flatMap(r => r.esp32_boards || []).map(b => <option key={b.id} value={b.id}>{b.name} ({b.mac_address})</option>)}
                    </select>
                  </div>
                </div>
                <div className="grid-2">
                  <div className="form-group">
                    ...
                    <input className="form-input" placeholder="Ceiling Light" value={newDevice.name} onChange={e => setNewDevice({...newDevice, name: e.target.value})} required />
                  </div>
                  <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" type="number" min="0" max="40" placeholder="e.g. 5" value={newDevice.pin_number} onChange={e => setNewDevice({...newDevice, pin_number: e.target.value})} required />
                </div>
                <button type="submit" className="btn btn-primary btn-sm" style={{ marginBottom: 16 }}>+ Add Device</button>
              </form>
              <div className="flex-between">
                <button className="btn btn-secondary" onClick={() => setSetupStep(3)}>← Back</button>
                <button className="btn btn-success" onClick={() => { setShowSetup(false); fetchHouses(); }}>✓ Finish Setup</button>
              </div>
            </div>
          )}
        </div>
